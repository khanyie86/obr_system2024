...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        // Prepare and bind
        $stmt = $conn->prepare("SELECT admin_id, admin_initials, admin_surname, admin_email, admin_passcode, admin_phone FROM admin WHERE admin_email = ?");
        if ($stmt) {
            ...
            if ($stmt->num_rows > 0) {
                $stmt->bind_result($admin_id, $admin_initials, $admin_surname, $admin_email, $hashed_password, $admin_phone);
                $stmt->fetch();
                ...
                if (password_verify($password, $hashed_password)) {
                    ...
                    $_SESSION['admin_initials'] = $admin_initials;
                    $_SESSION['admin_surname'] = $admin_surname;
